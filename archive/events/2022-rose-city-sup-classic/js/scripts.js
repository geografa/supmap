...
mapboxgl.accessToken =
  "pk.eyJ1IjoiZ3JhZmEiLCJhIjoiY2lvOHdtcDVlMDMwN3ZibHpucHczemQ4dSJ9.xfu_0kXfz1Nymp2LIyLS4Q";

const map = new mapboxgl.Map({
  container: "map",
  projection: "globe",
  style: "mapbox://styles/grafa/cko9i7v5n0p8417rzmtdrh0yt",
  zoom: 1.9466794621990684,
  center: { lng: -122.6693, lat: 45.551355 },
  pitch: 70,
  bearing: 0,
});
...
map.on("load", async () => {
  ...
  const trackGeojson = await fetch(`./data/evan-route-pin.geojson`).then((d) =>
    d.json()
  );
...
const addPathSourceAndLayer = (trackGeojson) => {
  ...
  map.addSource("start-pin-top", {
    type: "geojson",
    data: createGeoJSONCircle(trackGeojson.geometry.coordinates[0], 0.025)
  });
  ...
  map.addSource("end-pin-top", {
    type: "geojson",
    data: createGeoJSONCircle(trackGeojson.geometry.coordinates.slice(-1)[0], 0.025)
  });
